...
function pagescroll() {
  ...
  window.addEventListener("scroll", function () {
    ...
    if ((pscroll < pageYOffset) & (pageYOffset > header.offsetHeight)) {
      header.style = "transform:translateY(-100%); box-shadow: none";  //прячем хедер при прокрутке вниз
    } else {
      header.style =
        "transform:none; box-shadow: 0px 3px 6px rgba(0, 0, 0, 0.161);"; //показываем хедер при прокрутке вверх
    }
		if (pageYOffset < 50) {
...
// blue card
const cardBlueItems = document.querySelectorAll('.card-blue')

function blueCardToggler () {
  cardBlueItems.forEach(item => {
    item.addEventListener('click', function() {
      let cardBlueContainer = item.querySelector('.card-blue__container')
      cardBlueContainer.classList.toggle('active')
    })
  })
}
if (cardBlueItems) {
